...
case "${1:-fullstack}" in
  ...
  replicaset)
    ...
    echo "Connection string: mongodb://admin:${ADMIN_PASSWORD:-admin}@localhost:${MONGODB_PORT:-27017}/mbrain?authSource=admin&replicaSet=rs0&directConnection=true"
    echo ""
    echo "Features available:"
  ...
  stop)
    echo "Stopping all Mbrain MongoDB services..."
