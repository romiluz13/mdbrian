...
echo "Starting Mdbrain MongoDB initialization..."
...
# Create Mdbrain admin user (optional, for authenticated access)
echo "Creating mdbrain admin user..."
ADMIN_SCRIPT=$(mktemp /tmp/init-admin-XXXXXX.js)
cat > "$ADMIN_SCRIPT" << 'ENDOFSCRIPT'
const mdbrainDb = db.getSiblingDB('mdbrain');
const adminPwd = process.env.ADMIN_PASSWORD || 'admin';
try {
  mdbrainDb.createUser({
    user: 'mdbrain',
    pwd: adminPwd,
    roles: [{ role: 'readWrite', db: 'mdbrain' }]
  });
  print('User mdbrain created successfully');
} catch (error) {
  if (error.code === 11000) {
    print('User mdbrain already exists');
  } else {
    print('Warning: Could not create mdbrain user: ' + error);
    throw error;
  }
}
ENDOFSCRIPT
...
echo "Mdbrain MongoDB initialization completed successfully."
