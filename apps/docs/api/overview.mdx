---
title: API Overview
description: Mbrain HTTP API endpoint reference.
icon: "unplug"
---

The Mbrain API server runs at `http://127.0.0.1:3847` by default (via `apps/api`).

The API is the app-facing surface of the Mbrain Company Brain framework. It can
...
Set `MBRAIN_API_KEY` for every shared or network-exposed deployment. If no
admin or scoped key is configured, `/v1` routes stay open for trusted local
development and the API logs a startup warning.

When `MBRAIN_API_KEY` is set, all requests must include a `Bearer` token:
