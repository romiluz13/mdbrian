...
describe("mongodb benchmark runner", () => {
	...
	it("builds an operations report with build identity, gates, and warnings", () => {
		const previousCommit = process.env.MDBRAIN_BUILD_COMMIT
		process.env.MDBRAIN_BUILD_COMMIT = "abc123"
		try {
			...
			if (previousCommit == null) {
				delete process.env.MDBRAIN_BUILD_COMMIT
			} else {
				process.env.MDBRAIN_BUILD_COMMIT = previousCommit
			}
		}
	})
