...
import * as authModule from "@mdbrain/lib"
...
function mockResolvedProviderKey(apiKey = "test-key") {
	vi.mocked(authModule.resolveApiKeyForProvider).mockReturnValue(apiKey)
}
...
async function createProviderWithFetch(
	...
	options: Partial<Parameters<typeof createGeminiEmbeddingProvider>[0]> & {
		model: string
	},
) {
...
describe("GEMINI_EMBEDDING_2_MODELS", () => {
	it("contains gemini-embedding-2-preview", () => {
		expect(GEMINI_EMBEDDING_2_MODELS.has("gemini-embedding-2-preview")).toBe(
			true,
		)
	})
})
...
describe("resolveGeminiOutputDimensionality", () => {
	it("returns undefined for non-v2 models", () => {
		expect(
			resolveGeminiOutputDimensionality("gemini-embedding-001"),
		).toBeUndefined()
		expect(
			resolveGeminiOutputDimensionality("text-embedding-004"),
		).toBeUndefined()
	})

	it("returns 3072 by default for v2 models", () => {
		expect(
			resolveGeminiOutputDimensionality("gemini-embedding-2-preview"),
		).toBe(3072)
	})

	it("accepts valid dimension values", () => {
		expect(
			resolveGeminiOutputDimensionality("gemini-embedding-2-preview", 768),
		).toBe(768)
		expect(
			resolveGeminiOutputDimensionality("gemini-embedding-2-preview", 1536),
		).toBe(1536)
		expect(
			resolveGeminiOutputDimensionality("gemini-embedding-2-preview", 3072),
		).toBe(3072)
	})

	it("throws for invalid dimension values", () => {
		expect(() =>
			resolveGeminiOutputDimensionality("gemini-embedding-2-preview", 512),
		).toThrow(/Invalid outputDimensionality 512/)
		expect(() =>
			resolveGeminiOutputDimensionality("gemini-embedding-2-preview", 1024),
		).toThrow(/Valid values: 768, 1536, 3072/)
	})
})
...
describe("gemini-embedding-2-preview provider", () => {
	...
	it("sanitizes and normalizes structured batch responses", async () => {
		const fetchMock = createGeminiBatchFetchMock(1, [
			0,
