...
type AuthModule = typeof import("@mdbrian/lib")
...
beforeEach(async () => {
	vi.resetModules()
	authModule = await import("@mdbrian/lib")
...
describe("embedding provider auto selection", () => {
	...
	it("selects the first available remote provider in auto mode", async () => {
		...
		for (const testCase of cases) {
			...
			mockPublicPinnedHostname()
			vi.mocked(authModule.resolveApiKeyForProvider).mockImplementation(
				(provider) => testCase.resolveApiKey(provider),
			)

			const result = await createAutoProvider()
			const provider = expectAutoSelectedProvider(
				result,
...
describe("local embedding ensureContext concurrency", () => {
	...
	it("retries initialization after a transient ensureContext failure", async () => {
		...
		await expect(provider.embedBatch(["first"])).rejects.toThrow(
...
describe("FTS-only fallback when no provider available", () => {
	...
	it("returns null provider when all requested auth paths fail", async () => {
		...
		for (const testCase of [
			...
			expect(result.requestedProvider, testCase.name).toBe(
				testCase.requestedProvider,
			)
			expect(result.fallbackFrom, testCase.name).toBe(testCase.fallbackFrom)
			expect(result.providerUnavailableReason, testCase.name).toContain(
