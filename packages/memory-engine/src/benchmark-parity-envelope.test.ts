...
describe("resolveDatasetSha256", () => {
	it("prefers MDBRAIN_BENCHMARK_DATASET_SHA env over computing from path", async () => {
		const envSha = "a".repeat(64)
		const original = process.env.MDBRAIN_BENCHMARK_DATASET_SHA
		process.env.MDBRAIN_BENCHMARK_DATASET_SHA = envSha
		try {
			...
			if (original === undefined) {
				delete process.env.MDBRAIN_BENCHMARK_DATASET_SHA
			} else {
				process.env.MDBRAIN_BENCHMARK_DATASET_SHA = original
			}
		}
	})
	...
	it("ignores env value that is not a 64-hex SHA and falls back to path compute", async () => {
		...
		const original = process.env.MDBRAIN_BENCHMARK_DATASET_SHA
		process.env.MDBRAIN_BENCHMARK_DATASET_SHA = "not-a-real-sha"
		try {
	...
	it("throws in strict mode when no env and no path — zero silent fallback", async () => {
		const originalEnv = process.env.MDBRAIN_BENCHMARK_DATASET_SHA
		const originalStrict = process.env.MDBRAIN_BENCHMARK_STRICT
		delete process.env.MDBRAIN_BENCHMARK_DATASET_SHA
		...
		try {
			...
			if (originalEnv === undefined) {
				...
				process.env.MDBRAIN_BENCHMARK_DATASET_SHA = originalEnv
			}
			if (originalStrict === undefined) {
				delete process.env.MDBRAIN_BENCHMARK_STRICT
			} else {
				process.env.MDBRAIN_BENCHMARK_STRICT = originalStrict
			}
		}
	})
	...
	it("accepts override datasetSha256 arg ahead of env and path", async () => {
		const original = process.env.MDBRAIN_BENCHMARK_DATASET_SHA
		process.env.MDBRAIN_BENCHMARK_DATASET_SHA = "b".repeat(64)
		try {
...
describe("resolveBenchmarkEmbeddingConfig", () => {
	...
	it("honors MDBRAIN_BENCHMARK_EMBEDDING_MODEL env override", () => {
		const original = process.env.MDBRAIN_BENCHMARK_EMBEDDING_MODEL
		process.env.MDBRAIN_BENCHMARK_EMBEDDING_MODEL = "voyage-4-large"
		try {
			const cfg = resolveBenchmarkEmbeddingConfig({
				numDimensions: 1024,
				quantization: "none",
			...
			if (original === undefined) {
				delete process.env.MDBRAIN_BENCHMARK_EMBEDDING_MODEL
			} else {
				process.env.MDBRAIN_BENCHMARK_EMBEDDING_MODEL = original
			}
		}
	})
})
