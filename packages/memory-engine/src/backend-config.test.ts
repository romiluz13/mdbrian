...
describe("resolveMemoryBackendConfig", () => {
	...
	it("resolves MongoDB recall profile from env and ignores invalid values", () => {
		vi.stubEnv("MDBRAIN_MONGODB_RECALL_PROFILE", "proof")
		try {
			const cfg = {
				agents: { defaults: { workspace: "/tmp/memory-test" } },
				memory: {
		...
		vi.stubEnv("MDBRAIN_MONGODB_RECALL_PROFILE", "not-real")
		try {
			const cfg = {
				agents: { defaults: { workspace: "/tmp/memory-test" } },
				memory: {
	...
	it("allows MDBRAIN_MONGODB_COLLECTION_PREFIX to override config for benchmark isolation", () => {
		vi.stubEnv("MDBRAIN_MONGODB_COLLECTION_PREFIX", "bench_run_")
		const cfg = {
			agents: { defaults: { workspace: "/tmp/memory-test" } },
			memory: {
				backend: "mongodb",
	...
	it("resolves mongodb URI from MDBRAIN_MONGODB_URI env var", () => {
		vi.stubEnv("MDBRAIN_MONGODB_URI", "mongodb://from-env:27017")
		try {
			const cfg = {
				agents: { defaults: { workspace: "/tmp/memory-test" } },
				memory: {
	...
	it("resolves MongoDB connection pool overrides from env", () => {
		vi.stubEnv("MDBRAIN_MONGODB_MAX_POOL_SIZE", "6")
		vi.stubEnv("MDBRAIN_MONGODB_MIN_POOL_SIZE", "0")
		vi.stubEnv("MDBRAIN_MONGODB_MAX_CONNECTING", "2")
		vi.stubEnv("MDBRAIN_MONGODB_MAX_IDLE_TIME_MS", "120000")
		vi.stubEnv("MDBRAIN_MONGODB_SOCKET_TIMEOUT_MS", "180000")
		vi.stubEnv("MDBRAIN_MONGODB_WAIT_QUEUE_TIMEOUT_MS", "30000")
		vi.stubEnv("MDBRAIN_MONGODB_CONNECT_TIMEOUT_MS", "30000")
		vi.stubEnv("MDBRAIN_MONGODB_SERVER_SELECTION_TIMEOUT_MS", "120000")
		vi.stubEnv("MDBRAIN_MONGODB_HEARTBEAT_FREQUENCY_MS", "5000")
		vi.stubEnv("MDBRAIN_MONGODB_SERVER_MONITORING_MODE", "poll")
		vi.stubEnv("MDBRAIN_MONGODB_NETWORK_FAMILY", "4")
		const cfg = {
			agents: { defaults: { workspace: "/tmp/memory-test" } },
			memory: {
				backend: "mongodb",
	...
	it("allows fusionMethod override via MDBRAIN_MONGODB_FUSION_METHOD env var", () => {
		vi.stubEnv("MDBRAIN_MONGODB_FUSION_METHOD", "js-merge")
		const cfg = {
			agents: { defaults: { workspace: "/tmp/memory-test" } },
			memory: {
				backend: "mongodb",
	...
	it("allows reranking.enabled override via MDBRAIN_RERANKING_ENABLED env var", () => {
		vi.stubEnv("MDBRAIN_RERANKING_ENABLED", "false")
		try {
			const cfg = {
				agents: { defaults: { workspace: "/tmp/memory-test" } },
				memory: {
	...
	it("logs warning when entityExtraction.method is 'llm'", () => {
		vi.stubEnv("MDBRAIN_MONGODB_URI", "mongodb://localhost:27017/test")
	...
	it("allows numCandidates override via MDBRAIN_NUM_CANDIDATES env var", () => {
		vi.stubEnv("MDBRAIN_NUM_CANDIDATES", "300")
		const cfg = {
			agents: { defaults: { workspace: "/tmp/memory-test" } },
			memory: {
				backend: "mongodb",
	...
	it("allows reranking.minScore override via MDBRAIN_RERANK_MIN_SCORE env var", () => {
		vi.stubEnv("MDBRAIN_RERANK_MIN_SCORE", "0.05")
		const cfg = {
			agents: { defaults: { workspace: "/tmp/memory-test" } },
			memory: {
				backend: "mongodb",
