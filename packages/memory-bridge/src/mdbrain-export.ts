 ...
 * bundle. The bundle is HMAC-SHA256 signed using `MDBRAIN_EXPORT_SIGNING_KEY`
...
export function signExportBundle(
	...
	if (!signingKey || signingKey.length === 0) {
		throw new Error(
			"signExportBundle: signing key (MDBRAIN_EXPORT_SIGNING_KEY) must be set and non-empty",
		)
	}
